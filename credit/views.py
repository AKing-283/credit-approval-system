...
class Home(APIView):
    def get(self, request):
        return HttpResponse("Welcome to the Credit-Management-System")
            ...
            try:
                ...
                if loan_approved:
                    with transaction.atomic():
                        ...
                        customer.current_debt = calculate_remaining_loan_balance(loan)
                        customer.save()
                ...
                return Response({
                    'error': f'An error occurred while processing the loan: {str(e)}'
                }, status=500)
        return Response(serializer.errors, status=400)
...
class ViewLoanView(APIView):
    def get(self, request, loan_id):
        try:
            ...
            loan = Loan.objects.select_related('customer').filter(loan_id=loan_id).first()
            ...
            return Response({
                'error': f'An error occurred while retrieving the loan: {str(e)}'
            }, status=500)
